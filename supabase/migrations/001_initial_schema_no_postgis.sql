-- Map Circle Viewer - Initial Database Schema (без PostGIS)
-- Version: 1.0.1
-- Date: 2025-12-06
...
-- POI Points (infrastructure points)
...
CREATE INDEX idx_poi_points_lat_lng ON poi_points(lat, lng);
...
CREATE POLICY "POI layers are viewable by authenticated users"
    ON poi_layers FOR SELECT TO authenticated USING (true);

-- POI Points: readable by all authenticated users
CREATE POLICY "POI points are viewable by authenticated users"
    ON poi_points FOR SELECT TO authenticated USING (true);

-- Maps policies
CREATE POLICY "Users can view their own maps"
    ON maps FOR SELECT TO authenticated
    USING (user_id = auth.uid() OR id IN (SELECT map_id FROM map_access WHERE user_id = auth.uid()));
...
CREATE POLICY "Map access viewable by participants"
    ON map_access FOR SELECT TO authenticated
    USING (user_id = auth.uid() OR map_id IN (SELECT id FROM maps WHERE user_id = auth.uid()));

CREATE POLICY "Map owner can manage access"
    ON map_access FOR ALL TO authenticated
...
CREATE POLICY "Groups viewable by map participants"
    ...
    USING (map_id IN (
        ...
        UNION SELECT map_id FROM map_access WHERE user_id = auth.uid()
    ));

CREATE POLICY "Groups editable by map owner and editors"
    ON groups FOR INSERT TO authenticated
    WITH CHECK (map_id IN (
        SELECT id FROM maps WHERE user_id = auth.uid()
        UNION SELECT map_id FROM map_access WHERE user_id = auth.uid() AND access_level IN ('owner', 'editor')
    ));
...
CREATE POLICY "Points viewable by map participants"
    ON points FOR SELECT TO authenticated
    USING (group_id IN (
        SELECT g.id FROM groups g JOIN maps m ON g.map_id = m.id WHERE m.user_id = auth.uid()
        UNION SELECT g.id FROM groups g JOIN map_access ma ON g.map_id = ma.map_id WHERE ma.user_id = auth.uid()
    ));

CREATE POLICY "Points editable by map owner and editors"
    ON points FOR INSERT TO authenticated
    WITH CHECK (group_id IN (
        SELECT g.id FROM groups g JOIN maps m ON g.map_id = m.id WHERE m.user_id = auth.uid()
        UNION SELECT g.id FROM groups g JOIN map_access ma ON g.map_id = ma.map_id WHERE ma.user_id = auth.uid() AND ma.access_level IN ('owner', 'editor')
    ));

CREATE POLICY "Points updatable by map owner and editors"
    ON points FOR UPDATE TO authenticated
    USING (group_id IN (
        SELECT g.id FROM groups g JOIN maps m ON g.map_id = m.id WHERE m.user_id = auth.uid()
        UNION SELECT g.id FROM groups g JOIN map_access ma ON g.map_id = ma.map_id WHERE ma.user_id = auth.uid() AND ma.access_level IN ('owner', 'editor')
    ));
...
CREATE POLICY "Points deletable by map owner and editors"
    ON points FOR DELETE TO authenticated
    USING (group_id IN (
        SELECT g.id FROM groups g JOIN maps m ON g.map_id = m.id WHERE m.user_id = auth.uid()
        UNION SELECT g.id FROM groups g JOIN map_access ma ON g.map_id = ma.map_id WHERE ma.user_id = auth.uid() AND ma.access_level IN ('owner', 'editor')
    ));
...
CREATE POLICY "Comments viewable by map participants"
    ...
    USING (point_id IN (
        SELECT p.id FROM points p JOIN groups g ON p.group_id = g.id JOIN maps m ON g.map_id = m.id WHERE m.user_id = auth.uid()
        UNION SELECT p.id FROM points p JOIN groups g ON p.group_id = g.id JOIN map_access ma ON g.map_id = ma.map_id WHERE ma.user_id = auth.uid()
    ));

CREATE POLICY "Comments insertable by map owner and editors"
    ON comments FOR INSERT TO authenticated
    WITH CHECK (user_id = auth.uid() AND point_id IN (
        SELECT p.id FROM points p JOIN groups g ON p.group_id = g.id JOIN maps m ON g.map_id = m.id WHERE m.user_id = auth.uid()
        UNION SELECT p.id FROM points p JOIN groups g ON p.group_id = g.id JOIN map_access ma ON g.map_id = ma.map_id WHERE ma.user_id = auth.uid() AND ma.access_level IN ('owner', 'editor')
    ));
...
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO map_access (map_id, user_id, access_level) VALUES (NEW.id, NEW.user_id, 'owner');
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER create_map_owner_access AFTER INSERT ON maps FOR EACH ROW EXECUTE FUNCTION create_owner_access();
